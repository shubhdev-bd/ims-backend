...
class Assignment(models.Model):
    ...
    consent_images = models.JSONField(
    ...
    return_request_pending = models.BooleanField(default=False)
    return_request_reason = models.TextField(
        blank=True, help_text="User's reason for return request"
    )
    return_request_remarks = models.TextField(
        blank=True, help_text="User's remarks about device condition"
    )
    return_request_rejection_reason = models.TextField(
        blank=True, help_text="Admin's reason for rejecting return"
    )
    return_request_submitted_at = models.DateTimeField(null=True, blank=True)
...
class TicketRequest(models.Model):
    ...
    device = models.ForeignKey(
        Device, on_delete=models.SET_NULL, null=True, blank=True, related_name="tickets"
    )
...
class InventoryAsset(models.Model):
    ...
    serial_number = models.CharField(
        ...
        unique=True,
        db_index=True,
        help_text="Unique identifier from CSV",
    )
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="pending_claim", db_index=True
    )
    condition = models.CharField(
        max_length=20, choices=CONDITION_CHOICES, default="good"
    )
    ...
    metadata = models.JSONField(
        default=dict, blank=True, help_text="Stores category-specific fields as JSON"
    )

    # Duplicate Detection
    csv_import_id = models.CharField(
        max_length=255, blank=True, help_text="CSV import batch identifier"
    )
...
def link_inventory_assets_for_employee(employee):
    ...
    ).filter(Q(assigned_user__isnull=True) | Q(assigned_user=employee))

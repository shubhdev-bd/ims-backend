...
class DeviceViewSet(viewsets.ModelViewSet):
    ...
    def mark_maintenance(self, request, pk=None):
        ...
        return Response(
            {"message": "Device marked as under maintenance", "device": serializer.data}
        )
    ...
    def mark_available(self, request, pk=None):
        ...
        serializer = self.get_serializer(device)
        return Response(
            {"message": "Device marked as available", "device": serializer.data}
        )
...
class AssignmentViewSet(viewsets.ModelViewSet):
    ...
    def return_device(self, request, pk=None):
        ...
        return Response(
            {"message": "Device returned successfully", "assignment": serializer.data}
        )
    ...
    def submit_consent(self, request, pk=None):
        ...
        try:
            ...
            admin_emails = list(
                Employee.objects.filter(role="admin", is_active=True).values_list(
                    "email", flat=True
                )
            )
            if admin_emails:
    ...
    def approve_consent(self, request, pk=None):
        ...
        serializer = self.get_serializer(assignment)
        return Response(
            {"message": "Consent approved successfully", "assignment": serializer.data}
        )

    @action(detail=True, methods=["post"])
    def submit_return_form(self, request, pk=None):
        """Submit return request by user - creates a pending return request for admin approval"""
        assignment = self.get_object()
        ...
        return_remarks = return_data.get("remarks", "")

        # Create a return request instead of immediately returning
        assignment.return_request_pending = True
        assignment.return_request_reason = return_reason
        assignment.return_request_remarks = return_remarks
        assignment.return_form_data = return_data
        assignment.return_request_submitted_at = timezone.now()
        assignment.save()

        # Notify admins about the pending return request
        try:
            admin_subject = f"Device Return Request - {assignment.employee.full_name}"
            device_details = f"""
            <strong>Device:</strong> {assignment.device.device_id} - {assignment.device.brand} {assignment.device.model}<br/>
            <strong>Employee:</strong> {assignment.employee.full_name} ({assignment.employee.email})<br/>
            <strong>Assigned Date:</strong> {assignment.assigned_date.strftime('%d-%b-%Y %H:%M') if assignment.assigned_date else 'N/A'}<br/>
            <strong>Reason for Return:</strong> {return_reason}<br/>
            <strong>Device Condition:</strong> {return_data.get('condition', 'Not specified')}<br/>
            <strong>Remarks:</strong> {return_remarks}
            """
            admin_html = f"""
            <p>A device return request has been submitted.</p>
            {device_details}
            <p><br/>Please check the Device Requests &amp; Undertakings section to review and approve/reject this request.</p>
            """
            admin_text = (
                f"Device Return Request from {assignment.employee.full_name}\n"
            ...
            logger.warning(f"Failed to send admin notification for return request: {e}")
    ...
    def approve_return(self, request, pk=None):
        """Approve return request by admin - marks device as returned and sends confirmation email"""
        assignment = self.get_object()
        ...
        if not assignment.return_request_pending:
            return Response(
                {"error": "No pending return request to approve"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        # Mark return as approved and device as returned
        assignment.return_approved = True
        assignment.return_approved_at = timezone.now()
        assignment.return_approved_by = request.user
        assignment.status = "returned"
        assignment.return_date = timezone.now()
        assignment.return_request_pending = False
        assignment.save()

        # Send acceptance email to user with device details
        try:
            subject = f"Device Return Accepted - {assignment.device.device_id}"
            device_details = f"""
            <strong>Device ID:</strong> {assignment.device.device_id}<br/>
            <strong>Device:</strong> {assignment.device.brand} {assignment.device.model}<br/>
            <strong>Serial Number:</strong> {assignment.device.serial_number or 'N/A'}<br/>
            <strong>Device Type:</strong> {assignment.device.device_type.title()}<br/>
            <strong>Condition:</strong> {assignment.device.condition.title()}<br/>
            <strong>Assignment Date:</strong> {assignment.assigned_date.strftime('%d-%b-%Y') if assignment.assigned_date else 'N/A'}<br/>
            <strong>Return Date:</strong> {assignment.return_date.strftime('%d-%b-%Y') if assignment.return_date else 'N/A'}
            """
            html_body = f"""
            <p>Dear {assignment.employee.full_name},</p>
            <p>Your device return request has been <strong>accepted</strong>.</p>
            <p><strong>Device Details:</strong></p>
            {device_details}
            <p><br/>The device has been marked as returned in our system. Thank you for returning the device in good condition.</p>
            <p>If you have any questions, please contact the IT department.</p>
            <p>Best regards,<br/>Inventory Management System</p>
            """
            text_body = (
                f"Your device return request has been accepted.\n\n"
                f"Device: {assignment.device.device_id} - {assignment.device.brand} {assignment.device.model}\n"
                f"Serial: {assignment.device.serial_number or 'N/A'}\n"
                f"Return Date: {assignment.return_date.strftime('%d-%b-%Y') if assignment.return_date else 'N/A'}\n\n"
            ...
            logger.warning(f"Failed to send return acceptance email: {e}")
        ...
        return Response(
            {
                "message": "Return approved successfully. Confirmation email sent to user.",
                "assignment": serializer.data,
            }
        )

    @action(detail=True, methods=["post"])
    def reject_return_request(self, request, pk=None):
        """Reject return request by admin with reason"""
        assignment = self.get_object()

        if request.user.role not in ["admin", "manager"]:
            return Response(
                {"error": "Only admins and managers can reject returns"},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        rejection_reason = request.data.get("rejection_reason", "Not specified")

        # Store rejection reason and clear return_request_pending flag
        assignment.return_request_pending = False
        assignment.return_request_rejection_reason = rejection_reason
        assignment.save()

        # Send rejection email to user with reason
        try:
            subject = f"Device Return Request Rejected - {assignment.device.device_id}"
            device_details = f"""
            <strong>Device ID:</strong> {assignment.device.device_id}<br/>
            <strong>Device:</strong> {assignment.device.brand} {assignment.device.model}<br/>
            <strong>Serial Number:</strong> {assignment.device.serial_number or 'N/A'}
            """
            html_body = f"""
            <p>Dear {assignment.employee.full_name},</p>
            <p>Your device return request has been <strong>rejected</strong>.</p>
            <p><strong>Device Details:</strong></p>
            {device_details}
            <p><strong>Rejection Reason:</strong></p>
            <p>{rejection_reason}</p>
            <p><br/>Please contact the IT department if you need further assistance or wish to discuss this decision.</p>
            <p>Best regards,<br/>Inventory Management System</p>
            """
            text_body = (
                f"Your device return request has been rejected.\n\n"
                f"Device: {assignment.device.device_id} - {assignment.device.brand} {assignment.device.model}\n"
            ...
            logger.warning(f"Failed to send return rejection email: {e}")

        serializer = self.get_serializer(assignment)
        return Response(
            {
                "message": "Return request rejected. Notification email sent to user.",
                "assignment": serializer.data,
            }
        )
    ...
    def grant_device(self, request, pk=None):
        ...
        recipients = {
            ...
            "admin_recipients": (
                ...
                if hasattr(settings, "ADMIN_EMAIL_RECIPIENTS")
                else []
            ),
        }
        ...
        # Send to employee
        email_service.send_generic_email(
            [recipients["employee"]], subject, text_body, html_body=html_body
        )
        ...
        if admin_cc_recipients:
            ...
            email_service.send_generic_email(
                admin_cc_recipients, admin_subject, admin_text, html_body=admin_html
            )
...
class TicketRequestViewSet(viewsets.ModelViewSet):
    ...
    def resolve(self, request, pk=None):
        ...
        serializer = self.get_serializer(ticket)
        return Response(
            {"message": "Ticket resolved successfully", "ticket": serializer.data}
        )
    ...
    def revoke(self, request, pk=None):
        ...
        serializer = self.get_serializer(ticket)
        return Response(
            {"message": "Ticket revoked successfully", "ticket": serializer.data}
        )

    @action(detail=False, methods=["get"])
    def my_tickets(self, request):
        """Get current user's tickets"""
        tickets = self.filter_queryset(self.queryset.filter(requested_by=request.user))
...
class DeviceRequestViewSet(viewsets.ModelViewSet):
    ...
    def _approve_device_request(self, request, device_request):
        if device_request.status != "pending":
            return Response(
                {"error": "Request is not pending"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        selected_device.status = "assigned"
        selected_device.save(update_fields=["status"])
    ...
    def reject(self, request, pk=None):
        ...
        serializer = self.get_serializer(device_request)
        return Response(
            {"message": "Device request rejected", "request": serializer.data}
        )
...
class DashboardViewSet(viewsets.ViewSet):
    ...
    def stats(self, request):
        ...
        active_assignments = (
            Assignment.objects.filter(status="active").count() + asset_assigned
        )
        ...
        asset_by_category = dict(
            InventoryAsset.objects.values("category")
            .annotate(count=Count("id"))
        ...
        stats_data = {
            ...
            "recent_assignments": AssignmentListSerializer(
                recent_assignments, many=True
            ).data,
            "recent_tickets": TicketRequestListSerializer(
...
class InventoryAssetViewSet(viewsets.ModelViewSet):
    ...
    def my_inventory(self, request):
        ...
        assets = link_inventory_assets_for_employee(request.user).order_by(
            "-created_at"
        )
    ...
    def pending_claims(self, request):
        ...
        assets = InventoryAsset.objects.filter(
            pending_claim=True, assigned_email__isnull=False
        ).order_by("-created_at")
            ...
            return Response(
                {
                    ...
                    "email_result": {
                        "success": email_sent,
                        "message": (
                            "Claim email handled by inventory signal."
                            if asset.assigned_email
                            else "No assigned email on this asset."
                        ),
                    },
                }
            )
    ...
    def claim(self, request, pk=None):
        ...
        if (
            not asset.assigned_email
            or asset.assigned_email.strip().lower()
        ...
        return Response(
            {"message": "Device claimed successfully", "asset": serializer.data}
        )
    ...
    def bulk_import(self, request):
        ...
        try:
            ...
            return Response(
                {"success": True, "message": "Import completed", "results": results}
            )
